...
## Контроль Repository Study

Если для Process Check или текущего анализа требуется установить контекст проекта посредством **Repository Study**, Process Check не создаёт отдельный механизм полноты и не заменяет существующий авторитетный Read Set.

Исполнитель обязан использовать существующий `governance/Repository_Study_Read_Set_v0.1.md` и его условие `READ_COMPLETE` в соответствии с действующим Governance-процессом.

В частности, нельзя считать Repository Study завершённым только потому, что прочитан канонический маршрут восстановления. Полный Repository Study и чтение recovery route являются разными операциями.

`READ_COMPLETE` может быть принят как завершение Repository Study только при выполнении условий существующего Read Set, включая:

- определённый репозиторий;
- ветку или commit/ref;
- заявленную область исследования;
- каждый файл этой области;
- SHA файла или blob, где доступен;
- статус чтения каждого файла;
- зафиксированные failed/unread файлы;
- итоговый статус завершения.

Process Check здесь выполняет только **execution-интеграцию существующей authority**: если Repository Study необходим, он направляет исполнение к Read Set и не позволяет заменить его предположением, памятью или неполному чтению.

## Широкий поиск за пределами Repository Study

Если задача требует более широкого repository-only поиска — например, проверки Issues, Pull Requests, терминологических вариантов, функциональных или структурных эквивалентов и практических прецедентов, — исполнитель должен применять обычное аналитическое рассуждение и явно сообщать границы выполненного поиска.

Более широкая концепция полноты поиска и снимка репозитория сохранена отдельно в `SEARCH_COVERAGE_AND_SNAPSHOT_PRINCIPLE.md`.

Эта концепция пока **не является авторитетным правилом Foundation** и не расширяет самостоятельно действующий Governance System.
